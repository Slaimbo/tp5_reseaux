...
char in(char* chaine, char car)
{
	int i;
	for(i = 0; chaine[i] != '\0' && chaine[i] != car; i++);
...
int main(int argc, char *argv[])
{
  ...
  if (in(argv[1], ':'))
  {
	...
	indic.ai_family = PF_INET6;
	indic.ai_socktype = SOCK_STREAM;
	indic.ai_protocol = 0;
	indic.ai_addrlen = 0;
	indic.ai_addr = NULL;
	indic.ai_canonname = NULL;
	indic.ai_next = NULL;
  ...
  if (connect(s,resol->ai_addr,len)<0) {
    perror("connexion");
    exit(4);
  }
